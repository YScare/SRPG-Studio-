...
使用方法；第一套素材，在职业显示条件中添加关键字为EX。
         第二套素材行走图放入effect文件夹，在职业显示条件中添加关键为EX2.并在自定义参数中写入{id2:x,id3:x,···,id10:x}x为effect素材的ID。
*/

//设置行走图的尺寸，在ss中，行走图的尺寸是地图尺寸的两倍，且最大尺寸为192，但是本插件没有限制。
var ZHENPIC_1 = 64;//第一套素材的尺寸
var ZHENPIC_2= 64;//第二套以后的素材的尺寸

//第二套素材的位置补正，方便与第一套对齐
var EX2_X = 32;//第二套以后的素材的x值补正
var EX2_Y = 32;//第二套以后的素材的y值补正
var ChipConf={
...
(function(){
	StockItemControl.sortItem=function() {
		var itemArray = this.getStockItemArray();
	
		StockItemControl.sortItem.itemArray.sort(
			function(item1, item2,limit3,limit4,limit5,limit6,limit7,limit8,limit9,limit10) {
				var id1, id2,id3,id4,id5,id6,id7,id8,id9,id10;
				var limit1, limit2,limit3,limit4,limit5,limit6,limit7,limit8,limit9,limit10;
				...
				if (id1 > id2||id2 > id3||id3 > id4||id4 > id5||id5 > id6||id6 > id7||id7 > id8||id8 > id9||id9 > id10) {
					return 1;
				}
				else if (id1 < id2||id2 < id3||id3 < id4||id4 < id5||id5 < id6||id6 < id7||id7 < id8||id8 < id9||id9 < id10) {
					return -1;
				}
				else {
					limit1 = item1.getLimit();
					limit2 = item2.getLimit();
					limit3 = item3.getLimit();
					limit3 = item4.getLimit();
					limit3 = item5.getLimit();
					limit3 = item6.getLimit();
					limit3 = item7.getLimit();
					limit3 = item8.getLimit();
					limit3 = item9.getLimit();
					limit3 = item10.getLimit();
	
					if (limit1 > limit2||limit2 > limit3||limit3 > limit4||limit4 > limit5||limit5 > limit6||limit6 > limit7||limit7 > limit8||limit8 > limit9||limit9 > limit10) {
						return 1;
					}
					else if (limit1 < limit2||limit2 < limit3||limit3 < limit4||limit4 < limit5||limit5 < limit6||limit6 < limit7||limit7 < limit8||limit8 < limit9||limit9 < limit10) {
						return -1;
					}
				}
...
CustomCharChipGroup._configureCustomCharChip = function(groupArray) {
	...
	groupArray.push(ChipEX3);
	groupArray.push(ChipEX4);
	groupArray.push(ChipEX5);
	groupArray.push(ChipEX6);
	groupArray.push(ChipEX7);
	groupArray.push(ChipEX8);
	groupArray.push(ChipEX9);
	groupArray.push(ChipEX10);
...
var ChipEX = defineObject(BaseCustomCharChip,
{
	...
	_getpic: function(handle, colorIndex, type) {
		...
		if(this.getKeyword()=='EX3'){
			var id=this._unit.getClass().custom.id3;
			if(list.getCollectionDataFromId(id,colorIndex)==null){
				root.msg("画像がない!");root.endGame();
			}
			return list.getCollectionDataFromId(id, colorIndex);
		...
		if(this.getKeyword()=='EX4'){
			var id=this._unit.getClass().custom.id4;
			if(list.getCollectionDataFromId(id,colorIndex)==null){
				root.msg("画像がない!");root.endGame();
			}
			return list.getCollectionDataFromId(id, colorIndex);
		...
		if(this.getKeyword()=='EX5'){
			var id=this._unit.getClass().custom.id5;
			if(list.getCollectionDataFromId(id,colorIndex)==null){
				root.msg("画像がない!");root.endGame();
			}
			return list.getCollectionDataFromId(id, colorIndex);
		...
		if(this.getKeyword()=='EX6'){
			var id=this._unit.getClass().custom.id6;
			if(list.getCollectionDataFromId(id,colorIndex)==null){
				root.msg("画像がない!");root.endGame();
			}
			return list.getCollectionDataFromId(id, colorIndex);
		...
		if(this.getKeyword()=='EX7'){
			var id=this._unit.getClass().custom.id7;
			if(list.getCollectionDataFromId(id,colorIndex)==null){
				root.msg("画像がない!");root.endGame();
			}
			return list.getCollectionDataFromId(id, colorIndex);
		...
		if(this.getKeyword()=='EX8'){
			var id=this._unit.getClass().custom.id8;
			if(list.getCollectionDataFromId(id,colorIndex)==null){
				root.msg("画像がない!");root.endGame();
			}
			return list.getCollectionDataFromId(id, colorIndex);
		...
		if(this.getKeyword()=='EX9'){
			var id=this._unit.getClass().custom.id9;
			if(list.getCollectionDataFromId(id,colorIndex)==null){
				root.msg("画像がない!");root.endGame();
			}
			return list.getCollectionDataFromId(id, colorIndex);
		...
		if(this.getKeyword()=='EX10'){
			var id=this._unit.getClass().custom.id10;
			if(list.getCollectionDataFromId(id,colorIndex)==null){
				root.msg("画像がない!");root.endGame();
			}
			return list.getCollectionDataFromId(id, colorIndex);
